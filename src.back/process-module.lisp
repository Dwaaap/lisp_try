...
(def-action-handler send-predefined-task-pred (action-desig pred_or_custom 
                                                task-name what value)
  (ros-info (task-pm) "Task send ~a" task-name)
  (call-relay-interface_pred pred_or_custom task-name what value )
)
...
(def-fact-group task-pm-action-designators (action-desig)
  ...
  ;(<- (action-desig ?desig (send-predefined-task-pred ?current-desig "predefined"
  ;                                                    ?task-name ?what ?value))
  ;    (task-pm-running?)
  ;    (desig:trajectory-desig? ?desig)
  ;    (current-designator ?desig ?current-desig)
  ;    (desig-prop ?current-desig (task-name ?task-name))
  ;    (desig-prop ?current-desig (p_or_c predefined))
  ;    (desig-prop ?current-desig (what ?what))
  ;    (desig-prop ?current-desig (value ?value))
  ;)
)
...
(def-fact-group task-process-module (matching-process-module available-process-module)
  ...
  (<- (matching-process-module ?desig task-process-module)
      ...
      (or (desig-prop ?desig (type trajectory))
          )
  )
