...
(def-top-level-cram-function simple-plan-2 (_name_)
  ...
  (with-designators 
    ((play_task (action `((type trajectory) (task-name ,_name_)))))
...
(def-top-level-cram-function modulable-plan (name)
  (ros-info (ack-plan) "Perform modulable task ~a" name)
  (with-process-modules-running (task-process-module)
  (with-designators 
    ((play_task (action `((type trajectory)       ; (p_or_c predefined)
                                                   (task-name ,name) 
                                                  ; (what nil)
                                                  ; (value nil)
                                                   ))))
...
(defun perform-experiment ()
  ...
  ;; Step 4 - Using a predefined task with modulable value
  ;;(modulable-plan "moveHand")
) 
;; Step 1: Simple plan - Use a predefined task
(defun send-predefined-task (name)
